...
export const animationLoopOptions = {
  ...
  onRender({gl, tick, aspect, cube, offScreen, framebuffer, useDevicePixels}) {
    cube.setUniforms({
      uTime: tick * 0.1,
    ...
    if (offScreen) {
      // Hack/xiaoji - gl.commit only works once unless we "reset" the context here
      // This might be a offscreen canvas context bug?
      gl.canvas.width += ((tick % 2) * 2 - 1);
    }
